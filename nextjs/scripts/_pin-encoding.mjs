...
const verifierAbi = [
  {
    type: "function", name: "verifyDecryptionEIP712KMSSignatures",
    inputs: [
...
const drawId = args[0], revealedSeed = args[1], weights = args[2], decryptionProof = args[3];
...
const draw = await publicClient.readContract({ address: POOL, abi: getDrawAbi, functionName: "getDraw", args: [drawId] });
const wHandles = [];
for (let i = 0; i < Number(draw.participantCount); i++) {
  wHandles.push(await publicClient.readContract({ address: POOL, abi: getDrawAbi, functionName: "drawWeightHandle", args: [drawId, BigInt(i)] }));
}
...
const candidates = {
  "abi.encode(uint64 seed, uint64 tw, uint64[] weights)": encodeAbiParameters(
    [u64, u64, { type: "uint64[]" }], [big(revealedSeed), totalWeight, weights.map(big)]),
  "abi.encode(uint64[] [seed,tw,...weights])": encodeAbiParameters(
    [{ type: "uint64[]" }], [[big(revealedSeed), totalWeight, ...weights.map(big)]]),
  "abi.encode(uint64[5] fixed)": encodeAbiParameters(
    [{ type: "uint64[5]" }], [[big(revealedSeed), totalWeight, ...weights.map(big)]]),
  "abi.encodePacked(seed,tw,w...)": concat(weights.map(w => encodeAbiParameters([u64], [big(w)]))),
  "abi.encodePacked(uint64[] arr)": encodeAbiParameters([{ type: "uint64[]" }], [[big(revealedSeed), totalWeight, ...weights.map(big)]]).slice(64), // hmm placeholder
};

// abi.encodePacked of dynamic arrays actually drops length+offset; do manually:
const flatPacked = concat([big(revealedSeed), totalWeight, ...weights.map(big)].map(w => encodeAbiParameters([u64], [w])));
candidates["abi.encodePacked flat words"] = flatPacked;

for (const [name, bytes] of Object.entries(candidates)) {
  try {
    await publicClient.readContract({
      address: KMS_VERIFIER, abi: verifierAbi,
