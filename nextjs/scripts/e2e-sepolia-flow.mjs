...
const vaultAbi = [
  ...
  { type: "function", name: "positionOf", inputs: [{ name: "account", type: "address" }], outputs: [{ type: "bytes32" }], stateMutability: "view" },
  ...
  { type: "function", name: "withdraw", inputs: [{ name: "encryptedAmount", type: "bytes32" }, { name: "inputProof", type: "bytes" }], outputs: [], stateMutability: "nonpayable" },
];
const tokenAbi = [
  { type: "function", name: "symbol", inputs: [], outputs: [{ type: "string" }], stateMutability: "view" },
  { type: "function", name: "decimals", inputs: [], outputs: [{ type: "uint8" }], stateMutability: "view" },
  { type: "function", name: "confidentialBalanceOf", inputs: [{ name: "account", type: "address" }], outputs: [{ type: "bytes32" }], stateMutability: "view" },
  ...
  { type: "function", name: "wrap", inputs: [{ name: "to", type: "address" }, { name: "amount", type: "uint256" }], outputs: [{ type: "uint256" }], stateMutability: "nonpayable" },
];
const underlyingAbi = [
...
try {
  ...
  let balHandle = await publicClient.readContract({ address: asset, abi: tokenAbi, functionName: "confidentialBalanceOf", args: [account.address] });
  ...
  if (balHandle === "0x" + "0".repeat(64)) {
    ...
    await txHash("mint", () => walletClient.writeContract({ address: USDT, abi: underlyingAbi, functionName: "mint", args: [account.address, WRAP_AMOUNT], gas: 200_000n }));
    // USDTMock quirk: a non-zero allowance must be reset to 0 before changing it.
    const allowance = await publicClient.readContract({ address: USDT, abi: underlyingAbi, functionName: "allowance", args: [account.address, asset] });
    if (allowance < WRAP_AMOUNT) {
      if (allowance > 0n) {
        await txHash("approve-reset", () => walletClient.writeContract({ address: USDT, abi: underlyingAbi, functionName: "approve", args: [asset, 0n], gas: 200_000n }));
      }
      await txHash("approve", () => walletClient.writeContract({ address: USDT, abi: underlyingAbi, functionName: "approve", args: [asset, WRAP_AMOUNT], gas: 200_000n }));
    } else {
      console.log("underlying allowance sufficient:", allowance.toString());
    }
    await txHash("wrap", () => walletClient.writeContract({ address: asset, abi: tokenAbi, functionName: "wrap", args: [account.address, WRAP_AMOUNT], gas: 10_000_000n }));
    balHandle = await publicClient.readContract({ address: asset, abi: tokenAbi, functionName: "confidentialBalanceOf", args: [account.address] });
    console.log("funded; balance handle:", balHandle);
  }

  // 2. operator approval
  let isOperator = await publicClient.readContract({ address: asset, abi: tokenAbi, functionName: "isOperator", args: [account.address, VAULT] });
  if (!isOperator) {
    const MAX48 = 2 ** 48 - 1;
    await txHash("setOperator", () => walletClient.writeContract({ address: asset, abi: tokenAbi, functionName: "setOperator", args: [VAULT, MAX48], gas: 200_000n }));
    isOperator = true;
  } else {
  ...
  const readPosHandle = () => publicClient.readContract({ address: VAULT, abi: vaultAbi, functionName: "positionOf", args: [account.address] });
  ...
  const enc = await relayer.encrypt({ values: [{ value: DEPOSIT, type: "euint64" }], contractAddress: VAULT, userAddress: account.address });
  console.log("encrypted input ready");
  await txHash("deposit", () => walletClient.writeContract({ address: VAULT, abi: vaultAbi, functionName: "deposit", args: [bytesToHex(enc.handles[0]), bytesToHex(enc.inputProof)], gas: 10_000_000n }));
  ...
  const encW = await relayer.encrypt({ values: [{ value: WITHDRAW, type: "euint64" }], contractAddress: VAULT, userAddress: account.address });
  await txHash("withdraw", () => walletClient.writeContract({ address: VAULT, abi: vaultAbi, functionName: "withdraw", args: [bytesToHex(encW.handles[0]), bytesToHex(encW.inputProof)], gas: 10_000_000n }));
  ...
  const participantCount = await publicClient.readContract({ address: POOL, abi: poolAbi, functionName: "participantCount" });
  ...
  console.log(`\nE2E RESULT:`, posClear === expectedAfterDeposit && posClear2 === expectedAfterWithdraw ? "PASS ✓ (encrypted deltas exact)" : `FAIL (expected ${expectedAfterDeposit} then ${expectedAfterWithdraw}, got ${posClear} then ${posClear2})`);
} catch (e) {
