...
const maskHandle = (h) =>
  !h || h.length < 12
    ? "0x0000••••••••0000"
    : `0x${h.slice(2, 6).toUpperCase()}••••••••${h.slice(-4).toUpperCase()}`;
const shortHex = (v) => `${v.slice(0, 6)}...${v.slice(-4)}`;

const client = createPublicClient({
  chain: sepolia,
  transport: http("https://ethereum-sepolia-rpc.publicnode.com", { timeout: 30000 }),
});
...
console.log(
  "vault.prizePool():",
  prizePool,
  prizePool.toLowerCase() === POOL.toLowerCase() ? "(matches deployment ✓)" : "(MISMATCH)",
);
...
for (const log of [...logs]
  .sort((a, b) => Number(b.args.drawId) - Number(a.args.drawId))
  .slice(0, 5)) {
