/**
 * Generates per-contract files in nextjs/contracts/ from foundry
 * build output + broadcast receipts.
 *
 * Inputs:
 *   contracts/out/<Name>.sol/<Name>.json              — compiled ABI
 *   contracts/broadcast/.../<chainId>/run-latest.json — deployed addr+block
 *
 * Outputs (one pair per contract; bundlers tree-shake unused ones):
 *   nextjs/contracts/<Name>.ts        — non-local chains (tracked)
 *   nextjs/contracts/<Name>.local.ts  — local 31337 overlay (gitignored)
 *
...
const FOUNDRY_DIR = path.join(REPO_ROOT, "contracts");
const OUT_DIR = path.join(FOUNDRY_DIR, "out");
const BROADCAST_DIR = path.join(FOUNDRY_DIR, "broadcast");
const TARGET_DIR = path.join(REPO_ROOT, "nextjs/contracts");
...
async function main() {
  ...
  if (contractNames.length === 0) {
    console.warn("⚠ No deployments found in contracts/broadcast/");
  }
