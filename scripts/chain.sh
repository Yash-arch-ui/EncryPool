...
# forge-fhevm is installed as a soldeer dependency of contracts. The
...
FORGE_FHEVM_DIR="$(find "$REPO_ROOT/contracts/dependencies" -maxdepth 1 -type d -name 'forge-fhevm-*' | head -1)"

if [[ -z "$FORGE_FHEVM_DIR" || ! -d "$FORGE_FHEVM_DIR" ]]; then
  echo "error: forge-fhevm not found under contracts/dependencies/" >&2
  echo "run: (cd contracts && forge soldeer install)" >&2
  exit 1
fi
